...
class EfficientGAN():
    def __init__(self, input_dim=0, latent_dim=32, output_dir=''):
        self.input_dim = int(input_dim)
        self.latent_dim = int(latent_dim)
        self.output_dir = output_dir

    # Train model
    def fit(self, X_train, epochs=5, batch_size=50, loss=tf.keras.losses.BinaryCrossentropy(),
        ...
        for i in range(epochs):
            ...
            num = gen_data.shape[0]
            print(f"num : {num}")
            width = int(math.sqrt(num))
            height = int(math.ceil(float(num)/width))
            shape = gen_data[1]
            shape = shape.reshape([28, 28])
            image = np.zeros((height*shape.shape[0], width*shape.shape[1]), dtype=gen_data.dtype)
            for index, img in enumerate(gen_data):
                i_width = int(index/width)
                j_height = index % width
                img = img * 127.5
                img = img.reshape([28, 28])
                image[i_width*shape.shape[0]:(i_width+1)*shape.shape[0], j_height*shape.shape[1]:(j_height+1)*shape.shape[1]] = img[:, :]
            plt.imshow(image)
            plt.savefig(f'{self.output_dir}/{str(i+1)}.png')
    ...
    def predict(self, X_test, weight=0.9, degree=1):
        ...
        print(f"reconstructs : {reconstructs.shape}")
        reconstructs_reshape = reconstructs[0] * 127.5
        reconstructs_reshape = reconstructs_reshape.reshape([28, 28])
        plt.imshow(reconstructs_reshape)
        plt.savefig(f'{self.output_dir}/reconstructs.png')
        ...
        '''
            ...
            the data generated by a well-trained Encoder and Generator can reproduce the testing-data.
        '''
    ...
    def get_discriminator(self, initializer=tf.keras.initializers.GlorotUniform()):
        ...
        outputs = Dense(1, activation='sigmoid', kernel_initializer=initializer, name='output')(conet)
